...
ClearPath is a working Customer Success MVP that compares customer requests with a product roadmap using Claude. Roadmap items can be maintained manually or synchronized from Notion.
...
`deal_value` is treated as euros. Similar requests are grouped using the normalized theme generated by Claude.
